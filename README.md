# Useful assets

This repository is intended to store information about useful resources according to my interests.
Speaking of useful resources, it could be anything:
* some interesting images
* links to websites with helpful information
* links to websites with useful tools
* links to the useful repositories
* etc.

## Table of Content

1. System Design
2. QA Automation
   * Tools to practice QA Automation
     - [DemoQA](https://demoqa.com/)
     - [Practice Automation](https://practice-automation.com/)
     - [SauceDemo E-Commerce](https://www.saucedemo.com/)
     - [Cypress Real World App](https://github.com/cypress-io/cypress-realworld-app)
     - [GitHub Users Search](https://gh-users-search.netlify.app/)
     - [Contact List Web App and API](https://thinking-tester-contact-list.herokuapp.com/)
     - [RealWorld Apps](https://github.com/realworld-apps/realworld)
     - [WebDriverUniversity.com](https://webdriveruniversity.com/)
3. .Net Ecosystem
   * Interesting Libraries
     - [Spectre.Console](https://spectreconsole.net/) - build beautiful console applications
4. Others:
   * Interesting Tools
     - [Accessibility Insights](https://accessibilityinsights.io/) - solve accessibility issues before they reach your customers.
